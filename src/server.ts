...
import {
  interactiveServer,
  interactiveServerAddMediaService,
} from './interactiveServer';
...
const showUsage = () => {
  ...
  logger.debug(
    '    The port to listen for incoming connections socket connections.\n\n'
  );
  logger.debug('  --listenHost <host> (optional, default: 0.0.0.0)');
  logger.debug(
    '    The host to listen for incoming connections socket connections.\n\n'
  );
  logger.debug('  --secret <string> (optional, default: none)');
  logger.debug(
    '    The secret to use for authenticating with the room server.\n\n'
  );
  logger.debug(
    '  --cert <path> (optional, default: ./certs/edumeet-demo-cert.pem)'
  );
  logger.debug('    The path to the certificate file used for socket.\n\n');
  logger.debug(
    '  --key <path> (optional, default: ./certs/edumeet-demo-key.pem)'
  );
  ...
  logger.debug(
    '    The IP address to be announced to clients for mediasoup transports.\n\n'
  );
  logger.debug(
    '  --initialAvailableOutgoingBitrate <bitrate> (optional, default: 600000)'
  );
  logger.debug(
    '    The initial available outgoing bitrate for mediasoup transports.\n\n'
  );
  logger.debug(
    '  --maxIncomingBitrate <bitrate> (optional, default: 10000000)'
  );
  logger.debug('    The max incoming bitrate for mediasoup transports.\n\n');
  logger.debug(
    '  --maxOutgoingBitrate <bitrate> (optional, default: 10000000)'
  );
  ...
  logger.debug(
    '  --numberOfWorkers <num> (optional, default: number of host cores)'
  );
...
(async () => {
  ...
  httpServer.listen({ port: listenPort, host: listenHost }, () =>
    logger.debug('httpsServer.listen() [port: %s]', listenPort)
  );
  ...
  socketServer.on('connection', (socket) => {
    logger.debug('socket connection [socketId: %s]', socket.id);

    const { secret: connectionSecret } = socket.handshake.query;

    if (connectionSecret !== secret) {
      logger.error('invalid secret [socketId: %s]', socket.id);
